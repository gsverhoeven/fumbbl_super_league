...
The Super League, FUMBBLs biggest ressurrection league, has opened applications for season two. The Super League is a tiered league that will this season use the WC Alicante Rules, but without stars.
Find more info on the group page and join our discord server!
To sign up, create a team via the link on the group page and apply it to the group.
...
In season 0, there were 77 coaches. 

In November 2022 the league was filled for the first time.

Du you like tournament style of bloodbowl?

Do you want to improve you skills or prove that you already belong in the world elite, or maybe even both?

Sign up for The Super League. Sign ups closes on the 19th.

The league is open to anyone who has the ambition to improve from any level or to compete at any level including absolute world elite in tournament blood bowl.

So far we are almost at the numbers from the warm up season (76), and I am sure we have many more coming this time around. 
...
discord started in march 2023, before the start of season two.
...
gp <- ggplot(coach_levels %>% filter(coach_name %in% coach_sel), aes(y = reorder(coach_name, -sort_order2), x = season)) +
  ...
  theme_bw() +
  theme(legend.position="none")  +
  ggtitle("Super League\n Season 3 top division coach history")

ggsave("output/s3_top_coaches.png", gp, width = 600, height = 800, units = "px", dpi =  150)
