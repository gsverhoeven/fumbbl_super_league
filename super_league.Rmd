...
coach_sel <- coach_levels %>% 
  filter(season == 3 & level %in% c(1,2))  %>%
  pull(coach_name)

gp <- ggplot(coach_levels %>% filter(coach_name %in% coach_sel), aes(y = reorder(coach_name, -level), x = season)) +
  geom_tile(aes(fill = -level), color = "white", lwd = 1.5, linetype = 1) +
  geom_text(aes(label = level), color = "white", size = 2) +
...
ggsave("rel_pro_coaches.png", gp, width = 400, height = 800, units = "px", dpi =  150)

gp
```


# So what races get promoted most:

```{r}
res <- all_mbt %>% 
  group_by(action, season, race_name, tsl_tier) %>%
  summarise(n_coaches = n_distinct(coach_name))

ggplot(res, aes(x = reorder(race_name, n_coaches), y = n_coaches, color = factor(action))) +
  geom_point(position = position_dodge(width = 0.9)) + coord_flip() + expand_limits(y = 0) +
  scale_size_area() + ggtitle("FUMBBL Super League actions by race ") +
  facet_wrap(~ season) +
  labs(y = "Win rate (%)", x = "") + geom_hline( yintercept = 1, col = "grey")
```
...
```
We can also calculate average tier by division.

```{r}
res <- all_mbt %>%
  group_by(season, level) %>%
  summarise(n_games = n(),
            avg_tier = mean(tsl_tier))

ggplot(res, aes(x = level, y = avg_tier, color = factor(season))) +
  geom_point(aes(size = n_games)) + coord_flip() + expand_limits(y = 0) +
  scale_size_area() + ggtitle("FUMBBL Super League average tiers by division ") +
  facet_wrap(~ season) + 
  labs(y = "Average tier", x = "") + geom_hline( yintercept =  1, col = "black")
```
We can also look at what levels the various races are playing.

```{r}
res <- all_mbt %>%
  filter(level > 0) %>%
  group_by(race_name, season, tsl_tier) %>%
  summarise(n_games = n(),
            avg_level = mean(level))

ggplot(res, aes(x = reorder(race_name, -avg_level), y = avg_level, color = factor(tsl_tier))) +
  geom_point(aes(size = n_games)) + coord_flip() + expand_limits(y = 0) +
  scale_size_area() + ggtitle("FUMBBL Super League average level by race ") +
  facet_wrap(~ season) +
  labs(y = "Average tier", x = "") + geom_hline( yintercept =  1, col = "black")
```
Ok, so coaches can change race from season to season. 
