...
res <- all_mbt %>%
  filter(season == 2 & coach_name %in% coach_sel) %>%
  group_by(race_name, tsl_tier) %>%
  summarise(n_games = n(),
            n_coaches = n()/7,
...
ggplot(res, aes(x = reorder(race_name, -avg_level), y = avg_level, color = factor(tsl_tier))) +
  geom_point(aes(size = n_coaches)) + coord_flip() + expand_limits(y = 0) +
  scale_size_area() + ggtitle("S2 Teams played by coaches that qualified for\n top three divisions in S3") +
