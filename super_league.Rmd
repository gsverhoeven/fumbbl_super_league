...
- tournament needs to be dropped.
...
```{r}
df_matches <- read.csv(file = "../fumbbl_datasets/datasets/v0.6/df_matches.csv")
...
```{r}
df_mbt <- read.csv(file = "../fumbbl_datasets/datasets/v0.6/df_mbt.csv")
df_mbt <- df_mbt %>%
  filter(group_id == 15615)  %>% # TSL
  filter(tournament_id != 57864) %>%
  filter(tournament_id != 59504)
...
```{r}
all_mbt <- all_mbt %>%
  left_join(df_mbt %>% select(tournament_id, match_id, team_id, home_cas, away_cas), by = c("tournament_id", "match_id", "team_id"))
