...
all_matches <- rbind(all_matches, all_matches2)
```

```{r}
all_matches <- all_matches %>%
  left_join(df_mbt %>% select(tournament_id, match_id, team_id, coach_id, coach_name, home_cas, away_cas), by = c("tournament_id", "match_id", "team_id"))
  
```

add missing coach_ids and coachnames:

```{r}
# create full team/coach list
coaches <- all_matches %>% 
  select(team_id, coach_id, coach_name) %>% 
  drop_na() %>% 
  distinct()

all_matches <- all_matches %>% 
  select(-coach_id, -coach_name) %>%
  left_join(coaches, by = "team_id")
```
...
```{r}
all_matches %>% filter(tournament_id == 58588) %>% 
  arrange(round) %>%
  group_by(coach_name) %>%
  summarise(G = n(), 
